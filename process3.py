...
def on_flow1_message(ch, method, properties, body):
    """
    Flow 1 handler:
    Process 3 receives the reflected message from Process 2 and completes the flow.
    """
    try:
        data = json.loads(body.decode("utf-8"))
        print(f"\n[Process 3] [Flow 1 FINAL RECEIVE] Picked up item #{data.get('item_id')}")
        print(f"             Initial Counter: {data.get('counter') - 10} -> Process 2 Reflected Counter: {data.get('counter')}")
        print(f"             Audit Trail:")
        for step in data.get("history", []):
            print(f"               - [{step.get('stage')}] at {step.get('timestamp')}: {step.get('status', step.get('modification'))}")
...
def on_flow2_message(ch, method, properties, body):
    ...
    and reflects it back to the queue (QUEUE_FLOW2_P3_REFLECTED).
    """
    try:
        data = json.loads(body.decode("utf-8"))
...
def main():
    ...
    print(f"[Process 3] Will reflect Flow 2 messages to: '{QUEUE_FLOW2_P3_REFLECTED}'")
